 ...
 */
async function sendEmail({ to, subject, html, emailType, providerId = null, forceLive = false }) {
  ...
  try {
    let res = await sendResendRequest({
      from: fromEmail,
    ...
    // If domain verification error occurred and fromEmail is not onboarding@resend.dev, try fallback
    ...
    if (isDomainUnverified && !fromEmail.includes('onboarding@resend.dev')) {
      ...
      console.warn(`[ResendEmailService:NOTICE] Custom domain not yet verified. Falling back to ${fallbackFrom} for ${emailType}`);
...
const ResendEmailService = {
   ...
   */
  async sendSubscriptionActivatedEmail({ to, providerName = 'Artisan', plan = 'Pro', price = '₦8,000/month', nextRenewalDate = 'Next Month', contactAllowance = 100, forceLive = false }) {
    ...
    return sendEmail({ to, subject, html, emailType: 'subscription_activated', forceLive });
  },

  /**
   * 2. Payment Successful Email (Renewal or Initial)
   */
  async sendPaymentSuccessfulEmail({ to, providerName = 'Artisan', amount = '₦8,000', plan = 'Pro', reference = '', nextRenewal = 'Next Month', isRenewal = false, forceLive = false }) {
    ...
    return sendEmail({ to, subject, html, emailType: 'payment_successful', forceLive });
  },
   ...
   */
  async sendPlanChangedEmail({ to, providerName = 'Artisan', oldPlan = 'Basic', newPlan = 'Pro', effectiveDate = 'Immediately', newPrice = '₦8,000/month', newAllowance = 100, forceLive = false }) {
    ...
    return sendEmail({ to, subject, html, emailType: 'plan_changed', forceLive });
  },
